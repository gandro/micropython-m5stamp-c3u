...
class SCD40:
        ...
        while True:
            ...
            while not ready:
                ...
                if self.pressure is not None:
                    await self._write_values(_SCD40_CMD_SET_AMBIENT_PRESSURE,
            ...
            co2, t_raw, rh_raw = await self._read_values(
                _SCD40_CMD_READ_MEASUREMENT, 3)
            self.co2 = co2
    ...
    async def _write_values(self, cmd, values=(), *, delay_ms=1):
        nvalues = len(values)
        buf = memoryview(bytearray(_SCD40_CMD_LEN +
                                   nvalues * _SCD40_FRAME_LEN))
