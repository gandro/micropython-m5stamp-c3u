...
class BH1750FVI:
    def __init__(self, i2c, addr=I2C_DEFAULT_ADDR, *,
        ...
        self.mode = mode
        if (resolution != RESOLUTION_HIGH and resolution != RESOLUTION_HIGH2 and
    ...
    def sensitivity(self, factor=None):
        ...
        cmd[0] = (_BH1750FVI_MTREG_SET_HI | (
            mtreg >> _BH1750FVI_MTREG_SET_HI_SHIFT))
        self.i2c.writeto(self.addr, cmd)
        cmd[0] = (_BH1750FVI_MTREG_SET_LO | (
            mtreg & _BH1750FVI_MTREG_SET_LO_MASK))
        self.i2c.writeto(self.addr, cmd)
